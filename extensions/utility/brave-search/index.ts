 ...
 * - A manual `/brave-search` command (no LLM turn) for interactive browsing
...
const BRAVE_FRESHNESS_VALUES = new Set<BraveFreshness>([
  "pd",
  "pw",
  "pm",
  "py",
]);
const BRAVE_FORMAT_VALUES = new Set<BraveFormat>([
  "one_line",
  "short",
  "raw_json",
]);
...
function normalizeBraveQuery(query: string): string {
  ...
  const hasSingleQuotes =
    trimmed.startsWith("'") && trimmed.endsWith("'") && trimmed.length >= 2;
  const unwrapped =
    hasDoubleQuotes || hasSingleQuotes ? trimmed.slice(1, -1) : trimmed;
...
function stripInlineUsageTag(text: string): {
  text: string;
  ...
  try {
    ...
    if (parsed && typeof parsed === "object")
      return { text: without, usage: parsed as Record<string, unknown> };
  } catch {
    // ignore
  }
...
async function braveGroundingChatCompletion(params: {
  ...
  answer: string;
  usageInline: Record<string, unknown> | null;
  usageFinal: unknown | null;
...
function makeTurndownService(): TurndownService {
  ...
  const getExt = (node: any): string => {
    ...
    const parent = node?.parentNode
      ? firstTag(node.parentNode as Element).match(
  ...
  turndownService.addRule("fenceAllPreformattedText", {
    ...
    replacement: (_content, node) => {
      ...
      const code = [...(node as any).childNodes]
...
function extractGithubRenderedMarkdown(
  html: string,
  url: string,
): string | undefined {
  const dom = new JSDOM(html, { url });
...
async function fetchBraveResults(params: {
  ...
  for (const apiKey of apiKeys) {
    ...
    if (
      isQuotaOrAuthError(response.status) &&
      apiKeys.indexOf(apiKey) < apiKeys.length - 1
    ) {
      continue;
    }
  ...
  if (results.length === 0) {
    ...
    if (Array.isArray(newsResults))
      pushResults(newsResults, (r) => r?.url ?? r?.url_source ?? "");
  }
...
function shouldAddWebDocWorkflowNudge(prompt: string): boolean {
  ...
  const mentionsDocs =
    /\b(readme|docs?|documentation|changelog|release notes|spec|api reference)\b/i.test(
      p,
    );
  ...
  return (
    hasUrl ||
    hasSiteOperator ||
    ((mentionsWebContext || mentionsCommonHosts) && mentionsDocs)
  );
}
...
function buildWebDocWorkflowNudge(
  baseSystemPrompt: string,
  includeGroundingHint: boolean,
): string {
  return (
    baseSystemPrompt +
    "\n\n[Web search workflow]\n" +
...
function shouldProcessMarkerQuery(query: string, now: number): boolean {
  ...
  if (last !== undefined && now - last < WS_MARKER_DEDUP_WINDOW_MS)
    return false;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("context", async (event) => {
    const filtered = event.messages.filter(
      (m: any) =>
        !(m?.role === "custom" && WS_CUSTOM_TYPES.includes(m?.customType)),
    );

    return filtered.length === event.messages.length
      ? undefined
      : { messages: filtered };
  });
  ...
  pi.registerCommand("brave-search", {
    ...
    handler: async (args, ctx) => {
      const { query, count, country, freshness, fetchContent } =
        parseWsArgs(args);
      if (!query) {
        ctx.ui.notify(
          "Usage: /brave-search <query> [--freshness pd|pw|pm|py] [-n N] [--country US] [--content]",
          "warning",
        );
        return;
      }
      ...
      try {
        ...
        ctx.ui.notify(
          `Brave Search failed: ${e?.message ?? String(e)}`,
          "error",
        );
        return;
      }
      ...
      const body =
        results.length > 0 ? formatResultsAsText(results) : "No results";
  ...
  pi.registerTool({
    ...
    label: "Brave Search",
    description:
      "Web search via Brave Search API. Returns snippets; use fetchContent=true for full markdown saved to file.",
    parameters: Type.Object({
      ...
      freshness: Type.Optional(
        Type.String({
          description:
            "Optional time filter: pd (day), pw (week), pm (month), py (year)",
        }),
      ),
      fetchContent: Type.Optional(
        Type.Boolean({
          description:
            "Fetch full page as markdown, save to disk. Output includes 'Saved: <path>' — read that file. URL as query fetches directly (no search).",
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
          ...
          try {
            ...
            return {
              content: [
                {
                  type: "text",
                  text: `Direct fetch failed: ${e?.message ?? String(e)}`,
                },
              ],
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Brave Search failed: ${e?.message ?? String(e)}`,
              },
            ],
            details: {
              query,
      ...
      if (format === "one_line") {
        ...
        const line = top
          ? `${top.title} — ${top.url}${top.age ? ` (${top.age})` : ""}`
          : "No results";
      ...
      const text =
        resultsFinal.length > 0
          ? formatResultsAsText(resultsFinal)
          : "No results";
      return {
  ...
  pi.registerTool({
    ...
    description:
      "Grounded answer via Brave AI Grounding (chat/completions) with optional citations",
    parameters: Type.Object({
      question: Type.String({ description: "Question to answer" }),
      enableResearch: Type.Optional(
        Type.Boolean({ description: "Enable research mode (default true)" }),
      ),
      enableCitations: Type.Optional(
        Type.Boolean({ description: "Ask for citations (default true)" }),
      ),
      enableEntities: Type.Optional(
        Type.Boolean({
          description: "Ask for entity extraction (default false)",
        }),
      ),
      maxAnswerChars: Type.Optional(
        Type.Integer({
          description: "Max characters of answer to return",
          minimum: 200,
          maximum: 10000,
        }),
      ),
    ...
    async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
      ...
      const maxAnswerChars = clampNumber(
        Number((params as any).maxAnswerChars ?? 2500),
        200,
        10000,
      );

      let result: Awaited<ReturnType<typeof braveGroundingChatCompletion>>;
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Brave Grounding failed: ${e?.message ?? String(e)}`,
            },
          ],
          details: {
            question,
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    try {
      ...
      if (ctx.hasUI)
        ctx.ui.notify(
          `Marker brave-search search failed: ${e?.message ?? String(e)}`,
          "error",
        );
      return;
    }
