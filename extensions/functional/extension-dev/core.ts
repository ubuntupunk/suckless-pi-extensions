...
export function setupExtensionDevTools(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute() {
      return {
        ...
        details: { success: true, version: VERSION },
      };
    },
    renderCall: (_, theme) =>
      new ToolCallHeader({ toolName: "Pi Version" }, theme),
    renderResult: (res: any, _, theme) =>
      new Text(theme.fg("accent", `Version: ${res.details?.version}`), 0, 0),
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, _p, signal, onUpdate, ctx) {
      ...
      let searchDir = cwd;
      while (true) {
        for (const [f, p] of Object.entries(LOCKFILES))
          if (fs.existsSync(path.join(searchDir, f))) {
            pm = p;
            lockfile = f;
            break;
          }
      ...
      return {
        content: [{ type: "text", text: `PM: ${pm}` }],
        details: { success: true, packageManager: pm, lockfile },
      };
    },
    renderCall: (_, theme) =>
      new ToolCallHeader({ toolName: "Detect PM" }, theme),
    renderResult: (res: any, _, theme) =>
      new Text(
        theme.fg("success", `PM: ${res.details?.packageManager || "unknown"}`),
        0,
        0,
      ),
  });
  ...
  pi.registerTool({
    ...
    async execute() {
      ...
      if (!piPath)
        return {
          content: [{ type: "text", text: "Pi not found" }],
          details: { success: false, docFiles: [] },
        };
      const docs = [
        "README.md",
        ...listFiles(path.join(piPath, "docs")).map((f) => `docs/${f}`),
        ...listFiles(path.join(piPath, "examples")).map((f) => `examples/${f}`),
    ...
    renderCall: (_, theme) =>
      new ToolCallHeader({ toolName: "Pi Docs" }, theme),
    renderResult: (res: any, _, theme) =>
      new Text(
        theme.fg(
          "accent",
          `Found ${(res.details?.docFiles as string[] | undefined)?.length ?? 0} docs`,
        ),
        0,
        0,
      ),
  ...
  const parseEntries = (content: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (m && /^v?\d+\.\d+/.test(m[1] || m[2] || ""))
        entries.push({ version: m[1] || m[2], lineStart: i });
    }
    for (let i = 0; i < entries.length; i++) {
      const e = entries[i];
      const end = entries[i + 1]?.lineStart || lines.length;
      const raw = lines
        .slice(e.lineStart + 1, end)
        .join("\n")
        .trim();
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      try {
        ...
        const entries = parseEntries(
          fs.readFileSync(path.join(piPath, "CHANGELOG.md"), "utf-8"),
        );
        const e = params.version
          ? entries.find((x: any) => x.version.includes(params.version!))
          : entries[0];
    ...
    renderCall: (a, theme) =>
      new ToolCallHeader(
        { toolName: "Pi Changelog", mainArg: a.version || "latest" },
        theme,
      ),
    renderResult: (res: any, _, theme) =>
      new Text(
        `${theme.fg("success", "Found version")}\n${res.details?.changelog?.content?.split("\n").slice(0, 5).join("\n") || ""}`,
        0,
        0,
      ),
  });
