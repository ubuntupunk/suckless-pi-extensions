...
function getContextName(cwd: string): string {
  ...
  if (parts.length <= MAX_BREADCRUMB_DEPTH) return `${rootName} > ${parts.join(" > ")}`;
...
export default async function coreToolsExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool<typeof GetCurrentTimeParams, TimeDetails>({
    ...
    async execute(_toolCallId, params) {
      ...
      return { content: [{ type: "text", text: `Date: ${details.date} (${details.day_of_week})\nTime: ${details.time}` }], details };
    },
    renderCall: (args, theme) => new ToolCallHeader({ toolName: "Current Time", optionArgs: args.format ? [{ label: "format", value: args.format }] : [] }, theme),
    renderResult: (result, _opts, theme) => new Text(`${theme.fg("dim", "Date:")} ${theme.fg("accent", result.details?.date)} (${result.details?.day_of_week})\n${theme.fg("dim", "Time:")} ${theme.fg("accent", result.details?.time)}`, 0, 0),
  });
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      try {
        ...
        if (stat.isDirectory()) return nativeLs.execute(toolCallId, { path: p }, signal, onUpdate);
      } catch {}
  ...
  pi.on("session_start", async (_event, ctx) => emitTitle(pi, `π: ${getContextName(ctx.cwd)}`));
  pi.on("session_switch", async (_event, ctx) => emitTitle(pi, `π: ${getContextName(ctx.cwd)}`));
  pi.on("agent_start", async (_event, ctx) => emitTitle(pi, `π: ${getContextName(ctx.cwd)} (thinking...)`));
  pi.on("tool_call", async (event, ctx) => { emitTitle(pi, `π: ${getContextName(ctx.cwd)} (${event.toolName})`); return undefined; });
  pi.on("agent_end", async (_event, ctx) => emitTitle(pi, `π: ${getContextName(ctx.cwd)}`));
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    const additions = discovered.map(f => ({ type: "text" as const, text: `Loaded subdirectory context from ${discoveryManager.prettyPath(f.path)}\n\n${f.content}` }));
