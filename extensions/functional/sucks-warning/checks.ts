...
export interface SucksWarningCheckResult {
  /** Whether we're in the sucks warning zone */
  inZone: boolean;
...
/**
 * Check if text matches "this sucks" patterns.
 */
export function matchesSucksWarningPatterns(text: string): boolean {
...
/**
 * Check if we've entered the sucks warning zone.
 * Combines quantitative (context usage) and qualitative (phrase patterns) checks.
 */
export function checkSucksWarning(
  ctx: ExtensionContext,
  messages: AgentMessage[],
): SucksWarningCheckResult {
  ...
  if (lastAssistant) {
    ...
    if (matchesSucksWarningPatterns(text)) {
